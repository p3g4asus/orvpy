...
def gendevice(devtype, host, mac, timeout):
  if devtype == 0: # SP1
    return sp1(host=host, mac=mac, timeout=timeout)
  if devtype == 0x2711: # SP2
    return sp2(host=host, mac=mac, timeout=timeout)
  ...
  if devtype == 0x2720: # SPMini
    ...
    return rm(host=host, mac=mac, timeout=timeout)
    ...
    return a1(host=host, mac=mac, timeout=timeout)
  elif devtype == 0x4EB5 or devtype == 0x4EF7: # MP1: 0x4eb5, honyar oem mp1: 0x4ef7
    return mp1(host=host, mac=mac, timeout=timeout)
  elif devtype == 0x2722: # S1 (SmartOne Alarm Kit)
    return S1C(host=host, mac=mac, timeout=timeout)
  elif devtype == 0x4E4D: # Dooya DT360E (DOOYA_CURTAIN_V2)
    return dooya(host=host, mac=mac, timeout=timeout)
  else:
    return device(host=host, mac=mac, timeout=timeout)
...
def discover(timeout=None, local_ip_address=None, timeout2 = 10):
  ...
  if timeout is None:
    ...
    return gendevice(devtype, host, mac, timeout2)
  else:
    while (time.time() - starttime) < timeout:
      ...
      devtype = responsepacket[0x34] | responsepacket[0x35] << 8
      mac = responsepacket[0x3a:0x40]
      dev = gendevice(devtype, host, mac, timeout2)
      devices.append(dev)
    return devices
...
class rm2(rm):
  def __init__ (self, timeout=10):
    device.__init__(self, None, None, timeout)
